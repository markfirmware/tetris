...
pub const Tetris = struct {
    callback_user_pointer: *allowzero u64,
    drawText: DrawTextFn,
    fillRectMvp: FillRectMvpFn,
    drawParticle: DrawParticleFn,
    projection: Mat4x4,
...
pub const board_width = grid_width * cell_size;
pub const board_height = grid_height * cell_size;
pub const board_left = margin_size;
pub const board_top = margin_size;
...
pub const DrawTextFn = fn(pointer: *allowzero u64, t: *Tetris, text: []const u8, left: i32, top: i32, size: f32) void;
pub const FillRectMvpFn = fn(pointer: *allowzero u64, t: *Tetris, color: Vec4, mvp: Mat4x4) void;
pub const DrawParticleFn = fn(pointer: *allowzero u64, t: *Tetris, p: Particle) void;
pub fn setCallbacks(pointer: *allowzero u64, t: *Tetris, callback1: DrawParticleFn, callback2: DrawTextFn, callback3: FillRectMvpFn) void {
    t.callback_user_pointer = pointer;
    t.drawParticle = callback1;
    t.drawText = callback2;
    t.fillRectMvp = callback3;
}

pub fn setRandomSeed(t: *Tetris, rand_seed: u32) void {
...
fn fillRect(t: *Tetris, color: Vec4, x: f32, y: f32, w: f32, h: f32) void {
    ...
    const mvp = t.projection.mult(model);
    t.fillRectMvp(t.callback_user_pointer, t, color, mvp);
}
...
fn drawCenteredText(t: *Tetris, text: []const u8) void {
    ...
    t.drawText(t.callback_user_pointer, t, text, draw_left, draw_top, 1.0);
}
...
pub fn draw(t: *Tetris) void {
    ...
    }
    {
        ...
        const score_label_width = font_char_width * @intCast(i32, score_text.len);
        t.drawText(t.callback_user_pointer, t, score_text, score_left + score_width / 2 - @divExact(score_label_width, 2), score_top + score_height / 2, 1.0);
    }
    {
        const text = "LEVEL:";
        const text_width = font_char_width * @intCast(i32, text.len);
        t.drawText(t.callback_user_pointer, t, text, level_display_left + level_display_width / 2 - text_width / 2, level_display_top + margin_size, 1.0);
    }
    {
        var text_buf: [20]u8 = undefined;
        const text = bufPrint(text_buf[0..], "{}", t.level) catch unreachable;
        const text_width = font_char_width * @intCast(i32, text.len);
        t.drawText(t.callback_user_pointer, t, text, level_display_left + level_display_width / 2 - @divExact(text_width, 2), level_display_top + level_display_height / 2, 1.0);
    }
    ...
    for (t.particles) |maybe_particle| {
        if (maybe_particle) |particle| {
            t.drawParticle(t.callback_user_pointer, t, particle);
        }
    }
}
...
pub fn nextFrame(t: *Tetris, elapsed: f64) void {
    ...
    if (t.screen_shake_elapsed < t.screen_shake_timeout) {
        ...
        if (t.screen_shake_elapsed >= t.screen_shake_timeout) {
            ...
            const offset = 0.0; //@floatCast(f32, amplitude * -c.sin(2.0 * PI * t.screen_shake_elapsed * rate));
